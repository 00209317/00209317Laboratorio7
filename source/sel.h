...
Matrix createLocalK(int e,mesh &m){
    ...
    float tau,kappa,lambda,ni,psi,delta,eta,Ae,J,D;
    ...
    // [ A+K  -I+H ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixG,matrixD,matrixI,matrixH;
    ...
    //productRealMatrix(delta*Ae/(8*D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixG);
...
Vector createLocalb(int e,mesh &m){
    ...
    zeroes(b0,6);
    //b0 es el resultado de g_matrix*f
    productMatrixVector(g_matrix,f,b0);
    //b resudultado de j/6*
